...
status: Phase 7 보안 반영 Check (116/116 완성, 96%, Critical 0·High 0 — Phase 8/9 배포 잔여)
...
match_rate_history:
  ...
  v17.0_2026-05-31_phase7_security: 96
gap_count: 12
gap_count_history:
  v1.0_initial: 71
  v2.0_sprint0_end: 54
...
## 🔄 v17.0 (2026-05-31) — Phase 7 보안 반영 Check · Match Rate 96%

> gap-detector 독립 재검증. 직전 v16.0(95%, M08 완료) 대비 실작동 결함 2건 해소로 **96%**.

| 항목 | 값 |
|---|---|
| **Match Rate** | **96%** (95% → 96%) |
| **차단 Gap** | **Critical 0 · High 0** ✅ |
| **기능 매핑** | 116/116 (100%) — 9모듈 전체 구현 완료 |
| **잔여 Gap** | 12건 (Med 일부 문서 정정 + Low Phase 9 배포·잔여 정리) |

**이번 사이클 해소 (실작동 결함 2건)**
- **G-M07-3/4 Stored XSS 정화**: `lib/sanitize.ts`(isomorphic-dompurify, 서버컴포넌트/jsdom) 신설 → 운영자 HTML 무정화 렌더 3곳(success-cases problem/process/result+policy_detail, about content_html) 정화. **frontend/src 전수 재검증 결과 무정화 `dangerouslySetInnerHTML`·`innerHTML` 0건 확정**. 런타임(script·onerror·javascript:·iframe 제거) 검증.
- **카카오맵 env 런타임 버그**: `docker-compose.yml`의 `NEXT_PUBLIC_KAKAO_MAP_API_KEY`(잉여 `_API`) → 코드/`prod compose`와 일치하는 `NEXT_PUBLIC_KAKAO_MAP_KEY`로 수정 + `frontend/.env.example` 신설. dev 환경 맵 미작동 해소.

**이미 구현 확인 (작업 불필요)**: G-SEC-8 nginx 보안헤더 7종(`uscp.prod.conf` L54-60) · G-UX-11 비번리셋 실 SMTP(`mailer.py`+`auth.py:282`) · G-MAP-13 맵 fallback(`KakaoMap.tsx` L95).

**잔여 비차단 Gap (전부 Low, 8/20 오픈 비차단)**
- G-M08-B / G-M06: 감사로그 purge·MOU 만료알림 **cron 미연결** (엔드포인트 존재, 스케줄러만 Phase 9 연결)
- G-M08-A: 로그인 성공 시 audit `actor_id=NULL` (구조상 정상, IP·UA·성공실패는 기록)
- `ProjectMembership.tsx:74` `role==='admin'` V1 잔재 (admin/layout은 operator 허용으로 수정 완료, 이 1곳 미정리)
- design §4.2 일부 모듈(M04/M05/M08) 엔드포인트 수 표기 정정 권장
- M08-10 WCAG 도구 측정(Lighthouse/axe) UAT 직전 수행

**8/20 정식 오픈 잔여 필수 (Phase 9 배포)**: ① cron 2종 연결 ② env Secrets 주입 ③ `alembic upgrade head`(0013 멱등 확인·0014·0015 적용) ④ WCAG 도구 측정.
